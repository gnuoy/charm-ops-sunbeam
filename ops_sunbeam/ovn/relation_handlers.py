...
class OVNRelationUtils:
    ...
    @property
    def cluster_remote_public_addrs(self) -> Iterator[str]:
        """Retrieve remote addresses bound to remote endpoint.

        :returns: addresses bound to remote endpoints.
        :rtype: Iterator[str]
        """
        return self._remote_addrs("public-bound-address")
    ...
    @property
    def db_public_nb_connection_strs(self) -> Iterator[str]:
        """Provide OVN Northbound OVSDB connection strings.
        ...
        """
        return self.db_connection_strs(
            self.cluster_remote_public_addrs, self.db_nb_port
        )

    @property
    def db_public_sb_connection_strs(self) -> Iterator[str]:
        """Provide OVN Southbound OVSDB connection strings.
        ...
        """
        return self.db_connection_strs(
            self.cluster_remote_public_addrs, self.db_sb_port
        )
    ...
    @property
    def cluster_public_addr(self) -> ipaddress.IPv4Address:
        """Retrieve local address bound to endpoint.

        :returns: IPv4 or IPv6 address bound to endpoint
        :rtype: str
        """
        addresses = self._endpoint_public_bound_addresses()
        if len(addresses) > 1:
            logger.debug("Found multiple public ingress addresses, picking first one")
            address = addresses[0]
        elif len(addresses) == 1:
            address = addresses[0]
        else:
            logger.debug("Found no public ingress addresses")
            address = None
        return address
    ...
    def _endpoint_public_bound_addresses(self) -> ipaddress.IPv4Address:
        """Retrieve local address bound to endpoint.
        ...
        self.interface.set_unit_data(
            {
                ...
                "public-bound-address": str(self.cluster_public_addr),
            }
        )
        ...
        ctxt.update(
            {
                ...
                "db_public_sb_connection_strs": self.db_public_sb_connection_strs,
                "db_public_nb_connection_strs": self.db_public_nb_connection_strs,
